...
import { iglesia, casonaSalon } from '../assets/images'

function VenueCard({ id, imgSrc, imgAlt, timeBadge, iconSvg, type, name, addr, arrival, btnClass, btnHref, btnLabel, solo }) {
  return (
    <div className={`venue-card${solo ? ' venue-card-solo' : ''}`} id={id}>
...
export default function S5Venues() {
  const { tipo } = useGuest()
  const recepcionOnly = tipo === 'recepcion'

  return (
    <>
      <p className="eyebrow" style={{ marginBottom: '.75rem' }}>Cómo llegar</p>
      <h2 className="sec-title" style={{ marginBottom: '1rem' }}>Los <em>lugares</em></h2>
